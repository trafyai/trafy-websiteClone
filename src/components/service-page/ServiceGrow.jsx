        ...
        <div className='service-grow'>
            <div className='service-grow-container'>
                <div className='service-grow-left'>
                    ...
                    <p>Bring your ideas to life with trafy AI integrated Gen AI app development team. Your growth partner at every level.</p>
                </div>
                <div className='service-grow-right'>
                    {ServiceGrowData && ServiceGrowData.map((item, index) => (
                        <div className="service-grow-accordion" onClick={() => toggleAccordion(index)} key={index}>
                            <div className="service-grow-accordion-title">
                                <h3>{item.title}</h3>
                                {/* <ExpandMoreIcon className={`accordion-icon ${accordionState[index]?.isRotated ? "rotated" : ""}`} /> */}
                            </div>
